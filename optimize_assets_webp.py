...
- Uses lossless WebP by default to avoid introducing new quality loss

Examples:
  python3 optimize_assets_webp.py
  python3 optimize_assets_webp.py --root src/assets --delete-originals
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert and optimize images under an assets directory into WebP."
    )
    parser.add_argument(
        "--root",
        type=Path,
        default=Path("src/assets"),
        help="Root directory to scan recursively. Default: src/assets",
    )
    parser.add_argument(
        "--delete-originals",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Only replace an existing .webp when the new file saves at least this many bytes.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def process_existing_webp(
    ...
    *,
    ...
    try:
        if not dry_run:
            write_webp(path, temp_path, allow_lossy=allow_lossy, quality=quality)
        ...
        if savings < min_bytes_saved:
            ...
            stats.bytes_after += before_size
            logging.info(
                "Keeping existing file: %s (new=%s, current=%s, saved=%s)",
                path,
...
def process_convertible(
    ...
    *,
    ...
    if same_file(path, target):
        stats.skipped += 1
        stats.bytes_after += before_size
        logging.info("Skipped self-targeting conversion for %s", path)
        return
    ...
    write_webp(path, target, allow_lossy=allow_lossy, quality=quality)
...
def main() -> int:
    ...
    if not root.is_dir():
        logging.error("Root path is not a directory: %s", root)
        return 2

    logging.info("Starting recursive asset optimization in %s", root)
    logging.info(
        "Mode: %s",
        "lossy webp" if args.allow_lossy else "lossless webp",
    )
